...
def split_biographies(raw_text, book_name=None):
    ...
    # 过滤掉非传记内容
    non_bio_keywords = ["前言", "目录", "总目", "凡例", "序", "引言", "说明", "二十四史：文白对照精华版"]
    filtered_lines = []
    skip_mode = False
    
    for line in lines:
        # 检查是否进入非传记内容
        if any(keyword in line for keyword in non_bio_keywords):
            skip_mode = True
            continue
            ...
            for p_id, pair in enumerate(pairs, start=1):  # 每个传记 para_id 重置
                ...
                for sent in split_events(pair["original"]):
                    ...
                    events.append({
                        ...
                        "para_id": p_id,
                        "bio_name": bio_name,
                        "person_all": "|".join(sorted(persons)),
                        "original_text": sent
                    })
                    eid += 1
